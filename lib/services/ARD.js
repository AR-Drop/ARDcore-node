 ...
 */
function ARDcoin(options) {
  if (!(this instanceof ARDcoin)) {
    return new ARDcoin(options);
  }
...
util.inherits(ARDcoin, Service);

ARDcoin.dependencies = [];

ARDcoin.DEFAULT_MAX_TXIDS = 1000;
ARDcoin.DEFAULT_MAX_HISTORY = 50;
ARDcoin.DEFAULT_SHUTDOWN_TIMEOUT = 15000;
ARDcoin.DEFAULT_ZMQ_SUBSCRIBE_PROGRESS = 0.9999;
ARDcoin.DEFAULT_MAX_ADDRESSES_QUERY = 10000;
ARDcoin.DEFAULT_SPAWN_RESTART_TIME = 5000;
ARDcoin.DEFAULT_SPAWN_STOP_TIME = 10000;
ARDcoin.DEFAULT_TRY_ALL_INTERVAL = 1000;
ARDcoin.DEFAULT_REINDEX_INTERVAL = 10000;
ARDcoin.DEFAULT_START_RETRY_INTERVAL = 5000;
ARDcoin.DEFAULT_TIP_UPDATE_INTERVAL = 15000;
ARDcoin.DEFAULT_TRANSACTION_CONCURRENCY = 5;
ARDcoin.DEFAULT_CONFIG_SETTINGS = {
  server: 1,
  whitelist: '127.0.0.1',
  txindex: 1,
  addressindex: 1,
...
ARDcoin.prototype._initDefaults = function(options) {
  /* jshint maxcomplexity: 15 */

  // limits
  this.maxTxids = options.maxTxids || ARDcoin.DEFAULT_MAX_TXIDS;
  this.maxTransactionHistory = options.maxTransactionHistory || ARDcoin.DEFAULT_MAX_HISTORY;
  this.maxAddressesQuery = options.maxAddressesQuery || ARDcoin.DEFAULT_MAX_ADDRESSES_QUERY;
  this.shutdownTimeout = options.shutdownTimeout || ARDcoin.DEFAULT_SHUTDOWN_TIMEOUT;

  // spawn restart setting
  this.spawnRestartTime = options.spawnRestartTime || ARDcoin.DEFAULT_SPAWN_RESTART_TIME;
  this.spawnStopTime = options.spawnStopTime || ARDcoin.DEFAULT_SPAWN_STOP_TIME;

  // try all interval
  this.tryAllInterval = options.tryAllInterval || ARDcoin.DEFAULT_TRY_ALL_INTERVAL;
  this.startRetryInterval = options.startRetryInterval || ARDcoin.DEFAULT_START_RETRY_INTERVAL;

  // rpc limits
  this.transactionConcurrency = options.transactionConcurrency || ARDcoin.DEFAULT_TRANSACTION_CONCURRENCY;

  // sync progress level when zmq subscribes to events
  this.zmqSubscribeProgress = options.zmqSubscribeProgress || ARDcoin.DEFAULT_ZMQ_SUBSCRIBE_PROGRESS;
};

ARDcoin.prototype._initCaches = function() {
...
ARDcoin.prototype._initClients = function() {
  var self = this;
  this.nodes = [];
  this.nodesIndex = 0;
 ...
 */
ARDcoin.prototype.getAPIMethods = function() {
  var methods = [
 ...
 */
ARDcoin.prototype.getPublishEvents = function() {
  return [
    {
...
ARDcoin.prototype.subscribe = function(name, emitter) {
  this.subscriptions[name].push(emitter);
  log.info(emitter.remoteAddress, 'subscribe:', 'ARDd/' + name, 'total:', this.subscriptions[name].length);
};

ARDcoin.prototype.unsubscribe = function(name, emitter) {
...
ARDcoin.prototype.subscribeBalance = function(emitter, addresses) {
    var self = this;
...
ARDcoin.prototype.unsubscribeBalance = function(emitter, addresses) {
    var self = this;
    if(!addresses) {
...
ARDcoin.prototype.unsubscribeBalanceAll = function(emitter) {
...
ARDcoin.prototype.subscribeAddress = function(emitter, addresses) {
  var self = this;
...
ARDcoin.prototype.unsubscribeAddress = function(emitter, addresses) {
  var self = this;
  if(!addresses) {
 ...
 */
ARDcoin.prototype.unsubscribeAddressAll = function(emitter) {
...
ARDcoin.prototype._getDefaultConfig = function() {
  var config = '';
  var defaults = ARDcoin.DEFAULT_CONFIG_SETTINGS;
...
ARDcoin.prototype._parseARDcoinConf = function(configPath) {
  var options = {};
...
ARDcoin.prototype._expandRelativeDatadir = function() {
...
ARDcoin.prototype._loadSpawnConfiguration = function(node) {
  ...
  _.extend(this.spawn.config, this._parseARDcoinConf(configPath));

  var networkConfigPath = this._getNetworkConfigPath();
  if (networkConfigPath && fs.existsSync(networkConfigPath)) {
    _.extend(this.spawn.config, this._parseARDcoinConf(networkConfigPath));
  }
...
ARDcoin.prototype._checkConfigIndexes = function(spawnConfig, node) {
  $.checkState(
...
ARDcoin.prototype._resetCaches = function() {
...
ARDcoin.prototype._tryAllClients = function(func, callback) {
  var self = this;
...
ARDcoin.prototype._wrapRPCError = function(errObj) {
  var err = new errors.RPCError(errObj.message);
  err.code = errObj.code;
  return err;
};

ARDcoin.prototype._initChain = function(callback) {
  var self = this;
...
ARDcoin.prototype._getDefaultConf = function() {
  var networkOptions = {
    rpcport: 8766
  };
...
ARDcoin.prototype._getNetworkConfigPath = function() {
  var networkPath;
  if (this.node.network === ARDcore.Networks.testnet) {
...
ARDcoin.prototype._getNetworkOption = function() {
  var networkOption;
  if (this.node.network === ARDcore.Networks.testnet) {
...
ARDcoin.prototype._zmqBlockHandler = function(node, message) {
  var self = this;
...
ARDcoin.prototype._rapidProtectedUpdateTip = function(node, message) {
  var self = this;
...
ARDcoin.prototype._updateTip = function(node, message) {
  var self = this;
    ...
    if(!self.node.stopping) {
      self.syncPercentage(function(err, percentage) {
        if (err) {
          ...
          log.info('ARDcoin Height:', self.height, 'Percentage:', percentage.toFixed(2));
        }
      });
    }
  }
...
ARDcoin.prototype._getAddressesFromTransaction = function(transaction) {
  var addresses = [];
...
ARDcoin.prototype._notifyBalanceSubscribers = function(txid, transaction) {
...
ARDcoin.prototype._notifyBalanceSubscriber = function (address, txid) {
    var self = this;
...
ARDcoin.prototype._notifyAddressTxidSubscribers = function(txid, transaction) {
  var addresses = this._getAddressesFromTransaction(transaction);
  for (var i = 0; i < addresses.length; i++) {
    var address = addresses[i];
...
ARDcoin.prototype._zmqTransactionHandler = function(node, message) {
  var self = this;
...
ARDcoin.prototype._checkSyncedAndSubscribeZmqEvents = function(node) {
  var self = this;
  var interval;
  ...
  checkAndSubscribe(function(err, synced) {
    ...
    if (!synced) {
      ...
      }, node._tipUpdateInterval || ARDcoin.DEFAULT_TIP_UPDATE_INTERVAL);
    }
  });

};

ARDcoin.prototype._subscribeZmqEvents = function(node) {
  var self = this;
...
ARDcoin.prototype._initZmqSubSocket = function(node, zmqUrl) {
...
ARDcoin.prototype._checkReindex = function(node, callback) {
  var self = this;
  var interval;
  function finish(err) {
    clearInterval(interval);
  ...
  if (node._reindex) {
    interval = setInterval(function() {
      node.client.getBlockchainInfo(function(err, response) {
        ...
        log.info('ARDcoin Core Daemon Reindex Percentage: ' + percentSynced.toFixed(2));
    ...
    }, node._reindexWait || ARDcoin.DEFAULT_REINDEX_INTERVAL);
  } else {
    callback();
  }
};

ARDcoin.prototype._loadTipFromNode = function(node, callback) {
  var self = this;
  node.client.getBestBlockHash(function(err, response) {
    if (err && err.code === -28) {
...
ARDcoin.prototype._stopSpawnedARDcoin = function(callback) {
  var self = this;
  var spawnOptions = this.options.spawn;
...
ARDcoin.prototype._spawnChildProcess = function(callback) {
  var self = this;
  ...
  self._stopSpawnedARDcoin(function(err) {
    if (err) {
      return callback(err);
    }
    ...
    self.spawn.process.once('exit', function(code) {
      if (!self.node.stopping) {
        log.warn('ARDcoin process unexpectedly exited with code:', code);
        ...
        setTimeout(function() {
          self._spawnChildProcess(function(err) {
            ...
            log.warn('ARDcoin process restarted');
          });
...
ARDcoin.prototype._connectProcess = function(config, callback) {
  var self = this;
  var node = {};
  var exitShutdown = false;
 ...
 */
ARDcoin.prototype.start = function(callback) {
  var self = this;
  ...
  async.series([
    ...
    if (self.nodes.length === 0) {
      return callback(new Error('ARDcoin configuration options "spawn" or "connect" are expected'));
    }
 ...
 * @param {Function} callback
 */
ARDcoin.prototype.isSynced = function(callback) {
 ...
 * @param {Function} callback
 */
ARDcoin.prototype.syncPercentage = function(callback) {
  var self = this;
...
ARDcoin.prototype._normalizeAddressArg = function(addressArg) {
 ...
 * @param {Function} callback
 */
ARDcoin.prototype.getAddressBalance = function(addressArg, options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
ARDcoin.prototype.getAddressUnspentOutputs = function(addressArg, options, callback) {
  var self = this;
...
ARDcoin.prototype._getBalanceFromMempool = function(deltas) {
  var satoshis = 0;
...
ARDcoin.prototype._getTxidsFromMempool = function(deltas) {
  var mempoolTxids = [];
...
ARDcoin.prototype._getHeightRangeQuery = function(options, clone) {
 ...
 * @param {Function} callback
 */
ARDcoin.prototype.getAddressTxids = function(addressArg, options, callback) {
  /* jshint maxstatements: 20 */
  var self = this;
  var queryMempool = _.isUndefined(options.queryMempool) ? true : options.queryMempool;
...
ARDcoin.prototype._getConfirmationsDetail = function(transaction) {
...
ARDcoin.prototype._getAddressDetailsForInput = function(input, inputIndex, result, addressStrings) {
  if (!input.address) {
    return;
  }
...
ARDcoin.prototype._getAddressDetailsForOutput = function(output, outputIndex, result, addressStrings) {
  if (!output.address) {
    return;
  }
...
ARDcoin.prototype._getAddressDetailsForTransaction = function(transaction, addressStrings) {
  var result = {
    addresses: {},
    satoshis: 0
  };
 ...
 * @param {Function} callback
 */
ARDcoin.prototype._getAddressDetailedTransaction = function(txid, options, next) {
  var self = this;
...
ARDcoin.prototype._getAddressStrings = function(addresses) {
  var addressStrings = [];
  for (var i = 0; i < addresses.length; i++) {
    var address = addresses[i];
...
ARDcoin.prototype._paginateTxids = function(fullTxids, fromArg, toArg) {
  var txids;
 ...
 * @param {Function} callback
 */
ARDcoin.prototype.getAddressHistory = function(addressArg, options, callback) {
  var self = this;
  var addresses = self._normalizeAddressArg(addressArg);
...
+ * @param {Function} callback
+ */
ARDcoin.prototype.getAddressesMempoolBalance = function(addressArg, options, callback) {
    var self = this;
    var addresses = self._normalizeAddressArg(addressArg);
 ...
 * @param {Function} callback
 */
ARDcoin.prototype.getAddressSummary = function(addressArg, options, callback) {
  var self = this;
  var summary = {};
  var queryMempool = _.isUndefined(options.queryMempool) ? true : options.queryMempool;
  var summaryTxids = [];
...
ARDcoin.prototype._maybeGetBlockHash = function(blockArg, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
ARDcoin.prototype.getRawBlock = function(blockArg, callback) {
...
ARDcoin.prototype.getJsonBlock = function(blockArg, callback) {
    var self = this;
 ...
 * @param {Function} callback
 */
ARDcoin.prototype.getBlockOverview = function(blockArg, callback) {
  var self = this;
...
/**
 * Will retrieve a block as a ARDcore object
 * @param {String|Number} block - A block hash or block height number
 * @param {Function} callback
 */
ARDcoin.prototype.getBlock = function(blockArg, callback) {
  // TODO apply performance patch to the RPC method for raw data
  var self = this;
 ...
 * @param {Function} callback
 */
ARDcoin.prototype.getBlockHashesByTimestamp = function(high, low, options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
ARDcoin.prototype.getBlockHeader = function(blockArg, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
ARDcoin.prototype.estimateFee = function(blocks, callback) {
  var self = this;
...
ARDcoin.prototype.estimateSmartFee = function(blocks, conservative, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
ARDcoin.prototype.sendTransaction = function(tx, options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
ARDcoin.prototype.getRawTransaction = function(txid, callback) {
  var self = this;
...
+ * @param {Function} callback
+ */
ARDcoin.prototype.getJsonRawTransaction = function(txid, callback) {
  var self = this;
...
/**
 * Will get a transaction as a ARDcore Transaction. Results include the mempool.
 * @param {String} txid - The transaction hash
 * @param {Boolean} queryMempool - Include the mempool
 * @param {Function} callback
 */
ARDcoin.prototype.getTransaction = function(txid, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
ARDcoin.prototype.getDetailedTransaction = function(txid, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
ARDcoin.prototype.getBestBlockHash = function(callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
ARDcoin.prototype.getSpentInfo = function(options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
ARDcoin.prototype.getInfo = function(callback) {
  var self = this;
...
ARDcoin.prototype.generateBlock = function(num, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
ARDcoin.prototype.stop = function(callback) {
...
ARDcoin.prototype.getBlockReward = function(height) {
 ...
 * @param {Function} callback
 */
ARDcoin.prototype.listUnspent = function(minConf, maxConf, addresses, callback) {
    var self = this;
 ...
 * @return {*}
 */
ARDcoin.prototype.getMiningInfo = function(callback) {
    var self = this;
...
ARDcoin.prototype.getNetworkHash = function(height, callback) {
    var self = this;
...
ARDcoin.prototype.getPeerInfo = function(callback) {
    var self = this;
