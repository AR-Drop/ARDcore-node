 ...
 */
function ARD(options) {
  if (!(this instanceof ARD)) {
    return new ARD(options);
  }
...
util.inherits(ARD, Service);

ARD.dependencies = [];

ARD.DEFAULT_MAX_TXIDS = 1000;
ARD.DEFAULT_MAX_HISTORY = 50;
ARD.DEFAULT_SHUTDOWN_TIMEOUT = 15000;
ARD.DEFAULT_ZMQ_SUBSCRIBE_PROGRESS = 0.9999;
ARD.DEFAULT_MAX_ADDRESSES_QUERY = 10000;
ARD.DEFAULT_SPAWN_RESTART_TIME = 5000;
ARD.DEFAULT_SPAWN_STOP_TIME = 10000;
ARD.DEFAULT_TRY_ALL_INTERVAL = 1000;
ARD.DEFAULT_REINDEX_INTERVAL = 10000;
ARD.DEFAULT_START_RETRY_INTERVAL = 5000;
ARD.DEFAULT_TIP_UPDATE_INTERVAL = 15000;
ARD.DEFAULT_TRANSACTION_CONCURRENCY = 5;
ARD.DEFAULT_CONFIG_SETTINGS = {
  server: 1,
  whitelist: '127.0.0.1',
  txindex: 1,
  addressindex: 1,
...
ARD.prototype._initDefaults = function(options) {
  /* jshint maxcomplexity: 15 */

  // limits
  this.maxTxids = options.maxTxids || ARD.DEFAULT_MAX_TXIDS;
  this.maxTransactionHistory = options.maxTransactionHistory || ARD.DEFAULT_MAX_HISTORY;
  this.maxAddressesQuery = options.maxAddressesQuery || ARD.DEFAULT_MAX_ADDRESSES_QUERY;
  this.shutdownTimeout = options.shutdownTimeout || ARD.DEFAULT_SHUTDOWN_TIMEOUT;

  // spawn restart setting
  this.spawnRestartTime = options.spawnRestartTime || ARD.DEFAULT_SPAWN_RESTART_TIME;
  this.spawnStopTime = options.spawnStopTime || ARD.DEFAULT_SPAWN_STOP_TIME;

  // try all interval
  this.tryAllInterval = options.tryAllInterval || ARD.DEFAULT_TRY_ALL_INTERVAL;
  this.startRetryInterval = options.startRetryInterval || ARD.DEFAULT_START_RETRY_INTERVAL;

  // rpc limits
  this.transactionConcurrency = options.transactionConcurrency || ARD.DEFAULT_TRANSACTION_CONCURRENCY;

  // sync progress level when zmq subscribes to events
  this.zmqSubscribeProgress = options.zmqSubscribeProgress || ARD.DEFAULT_ZMQ_SUBSCRIBE_PROGRESS;
};

ARD.prototype._initCaches = function() {
...
ARD.prototype._initClients = function() {
  var self = this;
  this.nodes = [];
  this.nodesIndex = 0;
 ...
 */
ARD.prototype.getAPIMethods = function() {
  var methods = [
 ...
 */
ARD.prototype.getPublishEvents = function() {
  return [
    {
...
ARD.prototype.subscribeBalance = function(emitter, addresses) {
    var self = this;
...
ARD.prototype.unsubscribeBalance = function(emitter, addresses) {
    var self = this;
    if(!addresses) {
...
ARD.prototype.unsubscribeBalanceAll = function(emitter) {
...
ARD.prototype.subscribeAddress = function(emitter, addresses) {
  var self = this;
...
ARD.prototype.unsubscribeAddress = function(emitter, addresses) {
  var self = this;
  if(!addresses) {
 ...
 */
ARD.prototype.unsubscribeAddressAll = function(emitter) {
...
ARD.prototype._getDefaultConfig = function() {
  var config = '';
  var defaults = ARD.DEFAULT_CONFIG_SETTINGS;
...
ARD.prototype._parseARDConf = function(configPath) {
  var options = {};
...
ARD.prototype._expandRelativeDatadir = function() {
...
ARD.prototype._loadSpawnConfiguration = function(node) {
  ...
  log.info('Using ARD config file:', configPath);
  ...
  _.extend(this.spawn.config, this._parseARDConf(configPath));

  var networkConfigPath = this._getNetworkConfigPath();
  if (networkConfigPath && fs.existsSync(networkConfigPath)) {
    _.extend(this.spawn.config, this._parseARDConf(networkConfigPath));
  }
...
ARD.prototype._checkConfigIndexes = function(spawnConfig, node) {
  $.checkState(
...
ARD.prototype._resetCaches = function() {
...
ARD.prototype._tryAllClients = function(func, callback) {
  var self = this;
...
ARD.prototype._wrapRPCError = function(errObj) {
  var err = new errors.RPCError(errObj.message);
  err.code = errObj.code;
  return err;
};

ARD.prototype._initChain = function(callback) {
  var self = this;
...
ARD.prototype._getDefaultConf = function() {
  var networkOptions = {
    rpcport: 2831
  };
...
ARD.prototype._getNetworkConfigPath = function() {
  var networkPath;
  if (this.node.network === ARDcore.Networks.testnet) {
...
ARD.prototype._getNetworkOption = function() {
  var networkOption;
  if (this.node.network === ARDcore.Networks.testnet) {
...
ARD.prototype._zmqBlockHandler = function(node, message) {
  var self = this;
...
ARD.prototype._rapidProtectedUpdateTip = function(node, message) {
  var self = this;
...
ARD.prototype._updateTip = function(node, message) {
  var self = this;
    ...
    if(!self.node.stopping) {
      self.syncPercentage(function(err, percentage) {
        if (err) {
          ...
          log.info('ARD Height:', self.height, 'Percentage:', percentage.toFixed(2));
        }
      });
    }
  }
...
ARD.prototype._getAddressesFromTransaction = function(transaction) {
  var addresses = [];
...
ARD.prototype._notifyBalanceSubscribers = function(txid, transaction) {
...
ARD.prototype._notifyBalanceSubscriber = function (address, txid) {
    var self = this;
...
ARD.prototype._notifyAddressTxidSubscribers = function(txid, transaction) {
  var addresses = this._getAddressesFromTransaction(transaction);
  for (var i = 0; i < addresses.length; i++) {
    var address = addresses[i];
...
ARD.prototype._zmqTransactionHandler = function(node, message) {
  var self = this;
...
ARD.prototype._checkSyncedAndSubscribeZmqEvents = function(node) {
  var self = this;
  var interval;
  ...
  checkAndSubscribe(function(err, synced) {
    ...
    if (!synced) {
      ...
      }, node._tipUpdateInterval || ARD.DEFAULT_TIP_UPDATE_INTERVAL);
    }
  });

};

ARD.prototype._subscribeZmqEvents = function(node) {
  var self = this;
...
ARD.prototype._initZmqSubSocket = function(node, zmqUrl) {
...
ARD.prototype._checkReindex = function(node, callback) {
  var self = this;
  var interval;
  function finish(err) {
    clearInterval(interval);
  ...
  if (node._reindex) {
    interval = setInterval(function() {
      node.client.getBlockchainInfo(function(err, response) {
        ...
        log.info('ARD Core Daemon Reindex Percentage: ' + percentSynced.toFixed(2));
    ...
    }, node._reindexWait || ARD.DEFAULT_REINDEX_INTERVAL);
  } else {
    callback();
  }
};

ARD.prototype._loadTipFromNode = function(node, callback) {
  var self = this;
  node.client.getBestBlockHash(function(err, response) {
    if (err && err.code === -28) {
...
ARD.prototype._stopSpawnedARD = function(callback) {
  var self = this;
  var spawnOptions = this.options.spawn;
  ...
  function stopProcess() {
    fs.readFile(pidPath, 'utf8', function(err, pid) {
      ...
      try {
        log.warn('Stopping existing spawned ARD process with pid: ' + pid);
        self._process.kill(pid, 'SIGINT');
      } catch(err) {
        if (err && err.code === 'ESRCH') {
          log.warn('Unclean ARD process shutdown, process not found with pid: ' + pid);
          return callback(null);
        } else if(err) {
          return callback(err);
        }
...
ARD.prototype._spawnChildProcess = function(callback) {
  var self = this;
  ...
  self._stopSpawnedARD(function(err) {
    if (err) {
      return callback(err);
    }

    log.info('Starting ARD process');
    ...
    self.spawn.process.once('exit', function(code) {
      if (!self.node.stopping) {
        log.warn('ARD process unexpectedly exited with code:', code);
        log.warn('Restarting ARD child process in ' + self.spawnRestartTime + 'ms');
        setTimeout(function() {
          self._spawnChildProcess(function(err) {
            if (err) {
              return self.emit('error', err);
            }
            log.warn('ARD process restarted');
          });
...
ARD.prototype._connectProcess = function(config, callback) {
  var self = this;
  var node = {};
  var exitShutdown = false;
 ...
 */
ARD.prototype.start = function(callback) {
  var self = this;
  ...
  async.series([
    ...
    if (self.nodes.length === 0) {
      return callback(new Error('ARD configuration options "spawn" or "connect" are expected'));
    }
 ...
 * @param {Function} callback
 */
ARD.prototype.isSynced = function(callback) {
 ...
 * @param {Function} callback
 */
ARD.prototype.syncPercentage = function(callback) {
  var self = this;
...
ARD.prototype._normalizeAddressArg = function(addressArg) {
 ...
 * @param {Function} callback
 */
ARD.prototype.getAddressBalance = function(addressArg, options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
ARD.prototype.getAddressUnspentOutputs = function(addressArg, options, callback) {
  var self = this;
...
ARD.prototype._getBalanceFromMempool = function(deltas) {
  var satoshis = 0;
...
ARD.prototype._getTxidsFromMempool = function(deltas) {
  var mempoolTxids = [];
...
ARD.prototype._getHeightRangeQuery = function(options, clone) {
 ...
 * @param {Function} callback
 */
ARD.prototype.getAddressTxids = function(addressArg, options, callback) {
  /* jshint maxstatements: 20 */
  var self = this;
  var queryMempool = _.isUndefined(options.queryMempool) ? true : options.queryMempool;
...
ARD.prototype._getConfirmationsDetail = function(transaction) {
...
ARD.prototype._getAddressDetailsForInput = function(input, inputIndex, result, addressStrings) {
  if (!input.address) {
    return;
  }
...
ARD.prototype._getAddressDetailsForOutput = function(output, outputIndex, result, addressStrings) {
  if (!output.address) {
    return;
  }
...
ARD.prototype._getAddressDetailsForTransaction = function(transaction, addressStrings) {
  var result = {
    addresses: {},
    satoshis: 0
  };
 ...
 * @param {Object} txid - A ARD transaction id
 * @param {Function} callback
 */
ARD.prototype._getAddressDetailedTransaction = function(txid, options, next) {
  var self = this;
...
ARD.prototype._getAddressStrings = function(addresses) {
  var addressStrings = [];
  for (var i = 0; i < addresses.length; i++) {
    var address = addresses[i];
...
ARD.prototype._paginateTxids = function(fullTxids, fromArg, toArg) {
  var txids;
 ...
 * @param {Function} callback
 */
ARD.prototype.getAddressHistory = function(addressArg, options, callback) {
  var self = this;
  var addresses = self._normalizeAddressArg(addressArg);
...
+ * @param {Function} callback
+ */
ARD.prototype.getAddressesMempoolBalance = function(addressArg, options, callback) {
    var self = this;
    var addresses = self._normalizeAddressArg(addressArg);
 ...
 * @param {Function} callback
 */
ARD.prototype.getAddressSummary = function(addressArg, options, callback) {
  var self = this;
  var summary = {};
  var queryMempool = _.isUndefined(options.queryMempool) ? true : options.queryMempool;
  var summaryTxids = [];
...
ARD.prototype._maybeGetBlockHash = function(blockArg, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
ARD.prototype.getRawBlock = function(blockArg, callback) {
...
ARD.prototype.getJsonBlock = function(blockArg, callback) {
    var self = this;
 ...
 * @param {Function} callback
 */
ARD.prototype.getBlockOverview = function(blockArg, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
ARD.prototype.getBlock = function(blockArg, callback) {
  // TODO apply performance patch to the RPC method for raw data
  var self = this;
 ...
 * @param {Function} callback
 */
ARD.prototype.getBlockHashesByTimestamp = function(high, low, options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
ARD.prototype.getBlockHeader = function(blockArg, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
ARD.prototype.estimateFee = function(blocks, callback) {
  var self = this;
...
ARD.prototype.estimateSmartFee = function(blocks, conservative, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
ARD.prototype.sendTransaction = function(tx, options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
ARD.prototype.getRawTransaction = function(txid, callback) {
  var self = this;
...
+ * @param {Function} callback
+ */
ARD.prototype.getJsonRawTransaction = function(txid, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
ARD.prototype.getTransaction = function(txid, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
ARD.prototype.getDetailedTransaction = function(txid, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
ARD.prototype.getBestBlockHash = function(callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
ARD.prototype.getSpentInfo = function(options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
ARD.prototype.getInfo = function(callback) {
  var self = this;
...
ARD.prototype.generateBlock = function(num, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
ARD.prototype.stop = function(callback) {
...
ARD.prototype.getBlockReward = function(height) {
 ...
 * @param {Function} callback
 */
ARD.prototype.listUnspent = function(minConf, maxConf, addresses, callback) {
    var self = this;
 ...
 * @return {*}
 */
ARD.prototype.getMiningInfo = function(callback) {
    var self = this;
...
ARD.prototype.getNetworkHash = function(height, callback) {
    var self = this;
...
ARD.prototype.getPeerInfo = function(callback) {
    var self = this;
