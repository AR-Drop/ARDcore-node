...
/**
 * Provides a friendly event driven API to ARDd in Node.js. Manages starting and
 * stopping ARDd as a child process for application support, as well as connecting
 * to multiple ARDd processes for server infrastructure. Results are cached in an
 * LRU cache for improved performance and methods added for common queries.
 *
 * @param {Object} options
 * @param {Node} options.node - A reference to the node
 */
function Ravencoin(options) {
  if (!(this instanceof Ravencoin)) {
    return new Ravencoin(options);
  }
...
util.inherits(Ravencoin, Service);

Ravencoin.dependencies = [];

Ravencoin.DEFAULT_MAX_TXIDS = 1000;
Ravencoin.DEFAULT_MAX_HISTORY = 50;
Ravencoin.DEFAULT_SHUTDOWN_TIMEOUT = 15000;
Ravencoin.DEFAULT_ZMQ_SUBSCRIBE_PROGRESS = 0.9999;
Ravencoin.DEFAULT_MAX_ADDRESSES_QUERY = 10000;
Ravencoin.DEFAULT_SPAWN_RESTART_TIME = 5000;
Ravencoin.DEFAULT_SPAWN_STOP_TIME = 10000;
Ravencoin.DEFAULT_TRY_ALL_INTERVAL = 1000;
Ravencoin.DEFAULT_REINDEX_INTERVAL = 10000;
Ravencoin.DEFAULT_START_RETRY_INTERVAL = 5000;
Ravencoin.DEFAULT_TIP_UPDATE_INTERVAL = 15000;
Ravencoin.DEFAULT_TRANSACTION_CONCURRENCY = 5;
Ravencoin.DEFAULT_CONFIG_SETTINGS = {
  server: 1,
  whitelist: '127.0.0.1',
  txindex: 1,
  addressindex: 1,
...
Ravencoin.prototype._initDefaults = function(options) {
  /* jshint maxcomplexity: 15 */

  // limits
  this.maxTxids = options.maxTxids || Ravencoin.DEFAULT_MAX_TXIDS;
  this.maxTransactionHistory = options.maxTransactionHistory || Ravencoin.DEFAULT_MAX_HISTORY;
  this.maxAddressesQuery = options.maxAddressesQuery || Ravencoin.DEFAULT_MAX_ADDRESSES_QUERY;
  this.shutdownTimeout = options.shutdownTimeout || Ravencoin.DEFAULT_SHUTDOWN_TIMEOUT;

  // spawn restart setting
  this.spawnRestartTime = options.spawnRestartTime || Ravencoin.DEFAULT_SPAWN_RESTART_TIME;
  this.spawnStopTime = options.spawnStopTime || Ravencoin.DEFAULT_SPAWN_STOP_TIME;

  // try all interval
  this.tryAllInterval = options.tryAllInterval || Ravencoin.DEFAULT_TRY_ALL_INTERVAL;
  this.startRetryInterval = options.startRetryInterval || Ravencoin.DEFAULT_START_RETRY_INTERVAL;

  // rpc limits
  this.transactionConcurrency = options.transactionConcurrency || Ravencoin.DEFAULT_TRANSACTION_CONCURRENCY;

  // sync progress level when zmq subscribes to events
  this.zmqSubscribeProgress = options.zmqSubscribeProgress || Ravencoin.DEFAULT_ZMQ_SUBSCRIBE_PROGRESS;
};

Ravencoin.prototype._initCaches = function() {
...
Ravencoin.prototype._initClients = function() {
  var self = this;
  this.nodes = [];
  this.nodesIndex = 0;
 ...
 */
Ravencoin.prototype.getAPIMethods = function() {
  var methods = [
 ...
 */
Ravencoin.prototype.getPublishEvents = function() {
  return [
    {
...
Ravencoin.prototype.subscribe = function(name, emitter) {
  this.subscriptions[name].push(emitter);
  log.info(emitter.remoteAddress, 'subscribe:', 'ARDd/' + name, 'total:', this.subscriptions[name].length);
};

Ravencoin.prototype.unsubscribe = function(name, emitter) {
  var index = this.subscriptions[name].indexOf(emitter);
  if (index > -1) {
    this.subscriptions[name].splice(index, 1);
  }
  log.info(emitter.remoteAddress, 'unsubscribe:', 'ARDd/' + name, 'total:', this.subscriptions[name].length);
};

Ravencoin.prototype.subscribeBalance = function(emitter, addresses) {
    var self = this;
    ...
    for(var i = 0; i < addresses.length; i++) {
        if (ARDcore.Address.isValid(addresses[i], this.node.network)) {
            addAddress(addresses[i]);
        }
    }

    log.info(emitter.remoteAddress, 'subscribe:', 'ARDd/addressbalance', 'total:', _.size(this.subscriptions.balance));
};

Ravencoin.prototype.unsubscribeBalance = function(emitter, addresses) {
    var self = this;
    if(!addresses) {
    ...
    log.info(emitter.remoteAddress, 'unsubscribe:', 'ARDd/addressbalance', 'total:', _.size(this.subscriptions.balance));
};

Ravencoin.prototype.unsubscribeBalanceAll = function(emitter) {
...
Ravencoin.prototype.subscribeAddress = function(emitter, addresses) {
  var self = this;
  ...
  log.info(emitter.remoteAddress, 'subscribe:', 'ARDd/addresstxid', 'total:', _.size(this.subscriptions.address));
};

Ravencoin.prototype.unsubscribeAddress = function(emitter, addresses) {
  var self = this;
  if(!addresses) {
  ...
  log.info(emitter.remoteAddress, 'unsubscribe:', 'ARDd/addresstxid', 'total:', _.size(this.subscriptions.address));
};
 ...
 */
Ravencoin.prototype.unsubscribeAddressAll = function(emitter) {
...
Ravencoin.prototype._getDefaultConfig = function() {
  var config = '';
  var defaults = Ravencoin.DEFAULT_CONFIG_SETTINGS;
...
Ravencoin.prototype._parseRavencoinConf = function(configPath) {
  var options = {};
...
Ravencoin.prototype._expandRelativeDatadir = function() {
...
Ravencoin.prototype._loadSpawnConfiguration = function(node) {
  /* jshint maxstatements: 25 */

  $.checkArgument(this.options.spawn, 'Please specify "spawn" in ARDd config options');
  $.checkArgument(this.options.spawn.datadir, 'Please specify "spawn.datadir" in ARDd config options');
  $.checkArgument(this.options.spawn.exec, 'Please specify "spawn.exec" in ARDd config options');

  this._expandRelativeDatadir();

  var spawnOptions = this.options.spawn;
  var configPath = path.resolve(spawnOptions.datadir, './ARD.conf');

  log.info('Using ARDcoin config file:', configPath);
  ...
  _.extend(this.spawn.config, this._parseRavencoinConf(configPath));

  var networkConfigPath = this._getNetworkConfigPath();
  if (networkConfigPath && fs.existsSync(networkConfigPath)) {
    _.extend(this.spawn.config, this._parseRavencoinConf(networkConfigPath));
  }
...
Ravencoin.prototype._checkConfigIndexes = function(spawnConfig, node) {
  $.checkState(
    spawnConfig.txindex && spawnConfig.txindex === 1,
    '"txindex" option is required in order to use transaction query features of ARDcore-node. ' +
  ...
  $.checkState(
    ...
    '"addressindex" option is required in order to use address query features of ARDcore-node. ' +
  ...
  $.checkState(
    ...
    '"spentindex" option is required in order to use spent info query features of ARDcore-node. ' +
  ...
  $.checkState(
    ...
    '"server" option is required to communicate to ARDd from ARDcore. ' +
      'Please add "server=1" to your configuration and restart'
  );

  $.checkState(
    spawnConfig.zmqpubrawtx,
    '"zmqpubrawtx" option is required to get event updates from ARDd. ' +
      'Please add "zmqpubrawtx=tcp://127.0.0.1:<port>" to your configuration and restart'
  );

  $.checkState(
    spawnConfig.zmqpubhashblock,
    '"zmqpubhashblock" option is required to get event updates from ARDd. ' +
  ...
  $.checkState(
    ...
    '"zmqpubrawtx" and "zmqpubhashblock" are expected to the same host and port in ARD.conf'
  );

  if (spawnConfig.reindex && spawnConfig.reindex === 1) {
    log.warn('Reindex option is currently enabled. This means that ARDd is undergoing a reindex. ' +
             ...
             'of ARDcore-node services will start.');
    node._reindex = true;
  }
};

Ravencoin.prototype._resetCaches = function() {
...
Ravencoin.prototype._tryAllClients = function(func, callback) {
  var self = this;
...
Ravencoin.prototype._wrapRPCError = function(errObj) {
  var err = new errors.RPCError(errObj.message);
  err.code = errObj.code;
  return err;
};

Ravencoin.prototype._initChain = function(callback) {
  var self = this;
    ...
    self.client.getBlock(response.result, function(err, response) {
      ...
      self.client.getBlockHash(0, function(err, response) {
        ...
        self.getRawBlock(blockhash, function(err, blockBuffer) {
          ...
          self.emit('ready');
          log.info('Ravencoin Daemon Ready');
          callback();
        });
      });
...
Ravencoin.prototype._getDefaultConf = function() {
  var networkOptions = {
    rpcport: 8766
  };
  if (this.node.network === ARDcore.Networks.testnet) {
    networkOptions.rpcport = 18766;
  }
  return networkOptions;
};

Ravencoin.prototype._getNetworkConfigPath = function() {
  var networkPath;
  if (this.node.network === ARDcore.Networks.testnet) {
    networkPath = 'testnet3/ARD.conf';
    if (this.node.network.regtestEnabled) {
      networkPath = 'regtest/ARD.conf';
    }
  }
  return networkPath;
};

Ravencoin.prototype._getNetworkOption = function() {
  var networkOption;
  if (this.node.network === ARDcore.Networks.testnet) {
...
Ravencoin.prototype._zmqBlockHandler = function(node, message) {
  var self = this;
  ...
  if (!self.zmqKnownBlocks.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.hashblock.length; i++) {
      this.subscriptions.hashblock[i].emit('ARDd/hashblock', message.toString('hex'));
    }
  }

};

Ravencoin.prototype._rapidProtectedUpdateTip = function(node, message) {
  var self = this;
...
Ravencoin.prototype._updateTip = function(node, message) {
  var self = this;
    ...
    if(!self.node.stopping) {
      self.syncPercentage(function(err, percentage) {
        if (err) {
          ...
          log.info('Ravencoin Height:', self.height, 'Percentage:', percentage.toFixed(2));
        }
      });
    }
  }
...
Ravencoin.prototype._getAddressesFromTransaction = function(transaction) {
  var addresses = [];
...
Ravencoin.prototype._notifyBalanceSubscribers = function(txid, transaction) {
...
Ravencoin.prototype._notifyBalanceSubscriber = function (address, txid) {
    var self = this;
    self.getAddressSummary(address, {noTxList: false}, function(err, data) {
        ...
        if (emitters) {
            for(var j = 0; j < emitters.length; j++) {
                emitters[j].emit('ARDd/addressbalance', {
                    address: address,
                    txid: txid,
...
Ravencoin.prototype._notifyAddressTxidSubscribers = function(txid, transaction) {
  var addresses = this._getAddressesFromTransaction(transaction);
  for (var i = 0; i < addresses.length; i++) {
    var address = addresses[i];
    if(this.subscriptions.address[address]) {
      ...
      for(var j = 0; j < emitters.length; j++) {
        emitters[j].emit('ARDd/addresstxid', {
          address: address,
          txid: txid
        });
      }
...
Ravencoin.prototype._zmqTransactionHandler = function(node, message) {
  var self = this;
  var hash = ARDcore.crypto.Hash.sha256sha256(message);
  ...
  if (!self.zmqKnownTransactions.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.rawtransaction.length; i++) {
      this.subscriptions.rawtransaction[i].emit('ARDd/rawtransaction', message.toString('hex'));
    }
...
Ravencoin.prototype._checkSyncedAndSubscribeZmqEvents = function(node) {
  var self = this;
  var interval;
  ...
  checkAndSubscribe(function(err, synced) {
    ...
    if (!synced) {
      ...
      }, node._tipUpdateInterval || Ravencoin.DEFAULT_TIP_UPDATE_INTERVAL);
    }
  });

};

Ravencoin.prototype._subscribeZmqEvents = function(node) {
  var self = this;
...
Ravencoin.prototype._initZmqSubSocket = function(node, zmqUrl) {
...
Ravencoin.prototype._checkReindex = function(node, callback) {
  var self = this;
  var interval;
  function finish(err) {
    clearInterval(interval);
  ...
  if (node._reindex) {
    interval = setInterval(function() {
      node.client.getBlockchainInfo(function(err, response) {
        ...
        log.info('Ravencoin Core Daemon Reindex Percentage: ' + percentSynced.toFixed(2));
    ...
    }, node._reindexWait || Ravencoin.DEFAULT_REINDEX_INTERVAL);
  } else {
    callback();
  }
};

Ravencoin.prototype._loadTipFromNode = function(node, callback) {
  var self = this;
  node.client.getBestBlockHash(function(err, response) {
    if (err && err.code === -28) {
...
Ravencoin.prototype._stopSpawnedRavencoin = function(callback) {
  var self = this;
  var spawnOptions = this.options.spawn;
  var pidPath = spawnOptions.datadir + '/ARDd.pid';
  ...
  function stopProcess() {
    fs.readFile(pidPath, 'utf8', function(err, pid) {
      ...
      try {
        log.warn('Stopping existing spawned ARDcoin process with pid: ' + pid);
        self._process.kill(pid, 'SIGINT');
      } catch(err) {
        if (err && err.code === 'ESRCH') {
          log.warn('Unclean ARDcoin process shutdown, process not found with pid: ' + pid);
          return callback(null);
        } else if(err) {
          return callback(err);
        }
...
Ravencoin.prototype._spawnChildProcess = function(callback) {
  var self = this;
  ...
  self._stopSpawnedRavencoin(function(err) {
    if (err) {
      return callback(err);
    }

    log.info('Starting ARDcoin process');
    ...
    self.spawn.process.once('exit', function(code) {
      if (!self.node.stopping) {
        log.warn('Ravencoin process unexpectedly exited with code:', code);
        log.warn('Restarting ARDcoin child process in ' + self.spawnRestartTime + 'ms');
        setTimeout(function() {
          self._spawnChildProcess(function(err) {
            if (err) {
              return self.emit('error', err);
            }
            log.warn('Ravencoin process restarted');
          });
    ...
    async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
      ...
      if (exitShutdown) {
        return callback(new Error('Stopping while trying to spawn ARDd.'));
      }
...
Ravencoin.prototype._connectProcess = function(config, callback) {
  var self = this;
  var node = {};
  var exitShutdown = false;
  ...
  async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
    ...
    if (exitShutdown) {
      return callback(new Error('Stopping while trying to connect to ARDd.'));
    }
 ...
 */
Ravencoin.prototype.start = function(callback) {
  var self = this;
  ...
  async.series([
    ...
    if (self.nodes.length === 0) {
      return callback(new Error('Ravencoin configuration options "spawn" or "connect" are expected'));
    }
 ...
 * @param {Function} callback
 */
Ravencoin.prototype.isSynced = function(callback) {
 ...
 * @param {Function} callback
 */
Ravencoin.prototype.syncPercentage = function(callback) {
  var self = this;
...
Ravencoin.prototype._normalizeAddressArg = function(addressArg) {
 ...
 * @param {String|Address|Array} addressArg - An address string, ARDcore address, or array of addresses
 * @param {Object} options
 * @param {Function} callback
 */
Ravencoin.prototype.getAddressBalance = function(addressArg, options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Ravencoin.prototype.getAddressUnspentOutputs = function(addressArg, options, callback) {
  var self = this;
  ...
  function transformUnspentOutput(delta) {
    var script = ARDcore.Script.fromAddress(delta.address);
    return {
...
Ravencoin.prototype._getBalanceFromMempool = function(deltas) {
  var satoshis = 0;
...
Ravencoin.prototype._getTxidsFromMempool = function(deltas) {
  var mempoolTxids = [];
...
Ravencoin.prototype._getHeightRangeQuery = function(options, clone) {
  if (options.start >= 0 && options.end >= 0) {
    if (options.end > options.start) {
      throw new TypeError('"end" is expected to be less than or equal to "start"');
    }
    if (clone) {
      // reverse start and end as the order in ARDcore is most recent to less recent
 ...
 * @param {Function} callback
 */
Ravencoin.prototype.getAddressTxids = function(addressArg, options, callback) {
  /* jshint maxstatements: 20 */
  var self = this;
  var queryMempool = _.isUndefined(options.queryMempool) ? true : options.queryMempool;
...
Ravencoin.prototype._getConfirmationsDetail = function(transaction) {
...
Ravencoin.prototype._getAddressDetailsForInput = function(input, inputIndex, result, addressStrings) {
  if (!input.address) {
    return;
  }
...
Ravencoin.prototype._getAddressDetailsForOutput = function(output, outputIndex, result, addressStrings) {
  if (!output.address) {
    return;
  }
...
Ravencoin.prototype._getAddressDetailsForTransaction = function(transaction, addressStrings) {
  var result = {
    addresses: {},
    satoshis: 0
  };
 ...
 * @param {Object} txid - A ARDcoin transaction id
 * @param {Function} callback
 */
Ravencoin.prototype._getAddressDetailedTransaction = function(txid, options, next) {
  var self = this;
...
Ravencoin.prototype._getAddressStrings = function(addresses) {
  var addressStrings = [];
  for (var i = 0; i < addresses.length; i++) {
    var address = addresses[i];
    if (address instanceof ARDcore.Address) {
...
Ravencoin.prototype._paginateTxids = function(fullTxids, fromArg, toArg) {
  var txids;
 ...
 * @param {Function} callback
 */
Ravencoin.prototype.getAddressHistory = function(addressArg, options, callback) {
  var self = this;
  var addresses = self._normalizeAddressArg(addressArg);
...
+ * @param {Function} callback
+ */
Ravencoin.prototype.getAddressesMempoolBalance = function(addressArg, options, callback) {
    var self = this;
    var addresses = self._normalizeAddressArg(addressArg);
 ...
 * @param {Function} callback
 */
Ravencoin.prototype.getAddressSummary = function(addressArg, options, callback) {
  var self = this;
  var summary = {};
  var queryMempool = _.isUndefined(options.queryMempool) ? true : options.queryMempool;
  var summaryTxids = [];
...
Ravencoin.prototype._maybeGetBlockHash = function(blockArg, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Ravencoin.prototype.getRawBlock = function(blockArg, callback) {
...
Ravencoin.prototype.getJsonBlock = function(blockArg, callback) {
    var self = this;
 ...
 * @param {Function} callback
 */
Ravencoin.prototype.getBlockOverview = function(blockArg, callback) {
  var self = this;
...
/**
 * Will retrieve a block as a Ravencore object
 * @param {String|Number} block - A block hash or block height number
 * @param {Function} callback
 */
Ravencoin.prototype.getBlock = function(blockArg, callback) {
  // TODO apply performance patch to the RPC method for raw data
  var self = this;
    ...
    if (cachedBlock) {
      ...
      self._tryAllClients(function(client, done) {
        client.getBlock(blockhash, false, function(err, response) {
          ...
          var blockObj = ARDcore.Block.fromString(response.result);
 ...
 * @param {Function} callback
 */
Ravencoin.prototype.getBlockHashesByTimestamp = function(high, low, options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Ravencoin.prototype.getBlockHeader = function(blockArg, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Ravencoin.prototype.estimateFee = function(blocks, callback) {
  var self = this;
...
Ravencoin.prototype.estimateSmartFee = function(blocks, conservative, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Ravencoin.prototype.sendTransaction = function(tx, options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Ravencoin.prototype.getRawTransaction = function(txid, callback) {
  var self = this;
...
+ * @param {Function} callback
+ */
Ravencoin.prototype.getJsonRawTransaction = function(txid, callback) {
  var self = this;
...
/**
 * Will get a transaction as a Ravencore Transaction. Results include the mempool.
 * @param {String} txid - The transaction hash
 * @param {Boolean} queryMempool - Include the mempool
 * @param {Function} callback
 */
Ravencoin.prototype.getTransaction = function(txid, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Ravencoin.prototype.getDetailedTransaction = function(txid, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Ravencoin.prototype.getBestBlockHash = function(callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Ravencoin.prototype.getSpentInfo = function(options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Ravencoin.prototype.getInfo = function(callback) {
  var self = this;
...
Ravencoin.prototype.generateBlock = function(num, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Ravencoin.prototype.stop = function(callback) {
  if (this.spawn && this.spawn.process) {
    ...
    this.spawn.process.once('exit', function(code) {
      if (!exited) {
        exited = true;
        if (code !== 0) {
          var error = new Error('ARDd spawned process exited with status code: ' + code);
          error.code = code;
    ...
    setTimeout(function() {
      if (!exited) {
        exited = true;
        return callback(new Error('ARDd process did not exit'));
      }
...
Ravencoin.prototype.getBlockReward = function(height) {
 ...
 * @param {Function} callback
 */
Ravencoin.prototype.listUnspent = function(minConf, maxConf, addresses, callback) {
    var self = this;
 ...
 * @return {*}
 */
Ravencoin.prototype.getMiningInfo = function(callback) {
    var self = this;
...
Ravencoin.prototype.getNetworkHash = function(height, callback) {
    var self = this;
...
Ravencoin.prototype.getPeerInfo = function(callback) {
    var self = this;
