...
/**
 * Will remove a service from ARDcore-node.json
...
/**
 * Will remove the Node.js service and from the ARDcore-node configuration.
 * @param {String} options.cwd - The current working directory
 * @param {String} options.dirname - The ARDcore-node configuration directory
...
function remove(options, done) {
  ...
  var ARDcoreConfigPath = path.resolve(configPath, 'ARDcore-node.json');
  var packagePath = path.resolve(configPath, 'package.json');

  if (!fs.existsSync(ARDcoreConfigPath) || !fs.existsSync(packagePath)) {
    return done(
      new Error('Directory does not have a ARDcore-node.json and/or package.json file.')
    );
  }
  ...
  async.eachSeries(
    ...
    function(service, next) {
      ...
      removeService(configPath, service, function(err) {
        ...
        // remove service to ARDcore-node.json
        removeConfig(ARDcoreConfigPath, service, next);
      });
    }, done
  );
}
