...
/**
 * Will return the path and default ARDcore-node configuration. It will search for the
 * configuration file in the "~/.ARDcore" directory, and if it doesn't exist, it will create one
...
function getDefaultConfig(options) {
  ...
  var defaultPath = path.resolve(process.env.HOME, './.ARDcore');
  var defaultConfigFile = path.resolve(defaultPath, './ARDcore-node.json');

  if (!fs.existsSync(defaultPath)) {
    mkdirp.sync(defaultPath);
  }

  var defaultServices = ['ARDd', 'web'];
