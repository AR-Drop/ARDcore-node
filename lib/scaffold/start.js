...
var RavencoreNode = require('../node');
var index = require('../');
...
function checkConfigVersion2(fullConfig) {
  ...
  if (!datadirUndefined || addressDefined || dbDefined) {
    ...
    console.warn('\nConfiguration file is not compatible with this version. \n' +
                 'A reindex for ARDd is necessary for this upgrade with the "reindex=1" ARD.conf option. \n' +
                 'There are changes necessary in both ARD.conf and ARDcore-node.json. \n\n' +
                 'To upgrade please see the details below and documentation at: \n' +
                 'https://github.com/underdarkskies/ARDcore-node/blob/master/docs/upgrade.md \n');
 ...
 * @param {Object} options.config - The parsed ARDcore-node.json configuration file
...
function start(options) {
  ...
  fullConfig.path = path.resolve(options.path, './ARDcore-node.json');
...
function loadModule(req, service) {
  try {
    // first try in the built-in ARDcore-node services directory
    ...
    if (servicePackage.ARDcoreNode) {
      serviceModule = service.name + '/' + servicePackage.ARDcoreNode;
    }
 ...
 * @param {Node} node - The Ravencore Node instance
 */
