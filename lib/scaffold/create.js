...
var BASE_PACKAGE = {
  description: 'A full ARD node build with ARDcore',
  repository: 'https://github.com/wkibbler/ARDcore',
  license: 'MIT',
  readme: 'README.md',
  dependencies: {
    'ARDcore-lib': 'wkibbler/ARDcore-lib',
    'ARDcore-node': 'wkibbler/ARDcore-node'
  }
};

/**
 * Will create a directory and ARD.conf file for ARD.
 * @param {String} dataDir - The absolute path
 * @param {Function} done - The callback function called when finished
 */
function createARDDirectory(datadir, done) {
...
/**
 * Will create a base ARDcore Node configuration directory and files.
 * @param {Object} options
 * @param {String} options.network - "testnet" or "livenet"
 * @param {String} options.datadir - The ARD database directory
...
function createConfigDirectory(options, configDir, isGlobal, done) {
  mkdirp(configDir, function(err) {
    ...
    try {
      fs.writeFileSync(configDir + '/ARDcore-node.json', configJSON);
      if (!isGlobal) {
...
/**
 * Will setup a directory with a ARDcore Node directory, configuration file,
 * ARD configuration, and will install all necessary dependencies.
 *
 * @param {Object} options
 * @param {String} options.cwd - The current working directory
 * @param {String} options.dirname - The name of the ARDcore node configuration directory
 * @param {String} options.datadir - The path to the ARD datadir
...
function create(options, done) {
  ...
  async.series([
    function(next) {
      // Setup the the ARDcore-node directory and configuration
    ...
    function(next) {
      // Setup the ARD directory and configuration
      if (!fs.existsSync(absDataDir)) {
        createARDDirectory(absDataDir, next);
      } else {
        next();
      }
    },
