...
var ARDd = require('../').services.Ravencoin({
  node: {
...
ARDd.on('ready', function() {
  ...
  async.series([
    function(next) {
      ...
      function ARDdGetBlockNative(deffered) {
        if (c >= hashesLength) {
          c = 0;
        }
      ...
      function ARDdGetBlockJsonRpc(deffered) {
        if (c >= hashesLength) {
          c = 0;
        }
        var hash = fixtureData.blockHashes[c];
      ...
      function ARDGetTransactionNative(deffered) {
        if (c >= txLength) {
          c = 0;
        }
        var hash = fixtureData.txHashes[c];
        ARDd.getTransaction(hash, true, function(err, tx) {
          if (err) {
            throw err;
          }
          deffered.resolve();
      ...
      function ARDGetTransactionJsonRpc(deffered) {
        if (c >= txLength) {
          c = 0;
        }
        var hash = fixtureData.txHashes[c];
      ...
      suite.add('ARDd getblock (native)', ARDdGetBlockNative, {
        defer: true,
        maxTime: maxTime
      });

      suite.add('ARDd getblock (json rpc)', ARDdGetBlockJsonRpc, {
        defer: true,
        maxTime: maxTime
      });

      suite.add('ARDd gettransaction (native)', ARDGetTransactionNative, {
        defer: true,
        maxTime: maxTime
      });

      suite.add('ARDd gettransaction (json rpc)', ARDGetTransactionJsonRpc, {
        defer: true,
        maxTime: maxTime
      });
